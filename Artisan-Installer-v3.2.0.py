...
VERSION = "3.2.0"                 # BUMP THIS for every release (the updater compares it)
...
# The base name of the release asset / repo file, e.g. "Artisan-Installer.exe".
# find_release_asset() (below) also matches a version baked into the actual
# filename, so a release asset named "Artisan-Installer-v1.2.0.exe" or
# "Artisan-Installer-2.0.1.exe" is found automatically - you don't need to
# update this setting for every release.
RELEASE_SCRIPT_NAME = "Artisan-Installer.exe"
...
DEFAULT_REMOVALS = ["shippy-ships", "ingeniumapi"]   # Modrinth slugs of mods to delete if found, e.g. ["sodium"]
...
class Config:
    ...
    synced_at: str = ""   # ISO 8601 UTC timestamp from the published config, if any
...
def describe_age(iso_timestamp):
    """'2026-09-23T18:04:12+00:00' -> 'synced 5 minutes ago'. '' if blank/unparseable."""
    if not iso_timestamp:
        return ""
    try:
        synced = _dt.datetime.fromisoformat(iso_timestamp)
        if synced.tzinfo is None:
            synced = synced.replace(tzinfo=_dt.timezone.utc)
        seconds = max((_dt.datetime.now(_dt.timezone.utc) - synced).total_seconds(), 0)
    ...
    if minutes < 60:
        return f"synced {minutes} minute{'s' if minutes != 1 else ''} ago"
...
def apply_update(release):
    ...
    asset = find_release_asset(release["assets"], RELEASE_SCRIPT_NAME)
    if asset:
...
def apply_update_exe(release):
    ...
    asset = find_release_asset(release["assets"], RELEASE_SCRIPT_NAME)
    if not asset:
        raise UpdateError(f"the release has no asset matching '{RELEASE_SCRIPT_NAME}' "
                          f"(with or without a version in the filename)")
...
def print_intro(cfg: Config, mods_dir: Path):
    ...
    say(f"Installer version: {VERSION}")
    say()

    say(f"Minecraft version: {cfg.mc_version}", bold=True)
    say(f"Mod list loaded from: {cfg.source}")
    age = describe_age(cfg.synced_at)
    if age:
        say(age, indent=2, color="dim")
...
def print_plan(cfg, fplan, plans, rplans):
    ...
    labels = {
        "current": ("UP TO DATE", "bright_blue"),
    ...
    for p in plans:
        if p.action == "current":
            status_line("UP TO DATE", "bright_blue", f"{p.mod.name}: {p.installed_version} is already installed")
